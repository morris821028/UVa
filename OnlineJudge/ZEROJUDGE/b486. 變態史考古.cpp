...
LCT::Node *LCT::Node::EMPTY, *node_x, *node_rt;
...
int main() {
	...
	while (scanf("%d %d", &n, &m) == 2) {
		...
		for (int i = 0; i < m; i++) {
			...
			if (cmd[0] == 'n') {		// link
				scanf("%d %d", &x, &y);
				tree.link(tree.c(x), tree.c(y));
			} else if (cmd[0] == 's') {	// sim
				scanf("%d %d", &x, &y);
				node_x = tree.lca(tree.c(x), tree.c(y));
				int lca = tree.label(node_x);
				if (lca == 0) { 
					puts("-1");
				} else {
					node_rt = tree.find(tree.c(lca));
					int p1, p2;
					p1 = tree.path(tree.c(x), tree.c(y)) + 1;
					p2 = tree.path(tree.c(lca), node_rt) + 1;
