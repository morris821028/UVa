...
*/
	...
	*clMemOut = clCreateBuffer(*clCtx, clOutBuffFlag, sizeof(uint32_t)*8, NULL, &clStat);
	CheckFailAndExit(clStat);
...
int executeGPU(clFuncArgs, int N, uint32_t keyA, uint32_t keyB) {
	...
	// -- execute
	{
		...
		const int ZERO = 0;
        clStat = clEnqueueFillBuffer(*clQue, *clMemOut, &ZERO, sizeof(cl_uint), 0, sizeof(uint32_t)*8, 0, NULL, NULL);
        CheckFailAndExit(clStat);
	...
	// -- read back
	uint32_t sum[8], ret = 0;
	clEnqueueReadBuffer(*clQue, *clMemOut, CL_TRUE, 0, sizeof(uint32_t)*8, &sum, 0, NULL, NULL);
