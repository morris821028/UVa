...
struct Pt {
    double x, y;
    int cost;
    Pt(double a = 0, double b = 0): x(a), y(b) {cost = 0;}
...
struct AngleCmp {
	...
	bool operator() (const Pt& pa, const Pt& pb) {
		Pt p1 = pa - o, p2 = pb - o;
...
int bfs(map<Pt, vector<Pt>> &g, pair<Pt, Pt> st, pair<Pt, Pt> ed) {
	set<pair<Pt, Pt>> seen;
	vector<pair<Pt, Pt>> q;
    int ret = 0;
    q.push_back(st);
    while (true) {
    	vector<pair<Pt, Pt>> q2;
    	while (!q.empty()) {
    		auto u = q.back();
    		q.pop_back();
    		if (u == ed)
    		...
    		if (seen.count(u))
    			continue;
    		seen.insert(u);
    		pair<Pt, Pt> v(u.second, u.first);
    		if (u.second.cost) {
    			q2.push_back(v);
    		} else {
    			q.push_back(v);
    		}
    		const vector<Pt> &adj = g[u.second];
    		int idx = 0;
    		for (; idx < adj.size(); idx++) {
    			if (adj[idx] == u.first)
    				break;
			}
			v.second = adj[(idx+1)%adj.size()];
			q.push_back(v);
		}
    	q.swap(q2), ret++;
	}
