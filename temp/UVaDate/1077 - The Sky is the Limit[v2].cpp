...
double computeSkyline(Seg segs[], int n) {
	set<Event> events;
	double tot_len = 0;
	for (int i = 0; i < n; i++) {
...
int main() {
    ...
    while (scanf("%d", &N) == 1 && N) {
    	...
    	for (int i = 0; i < N; i++) {
    		...
    		segs[M] = Seg(Pt(lx, 0), Pt(X, H), M), M++;
    		segs[M] = Seg(Pt(rx, 0), Pt(X, H), M), M++;
//    		printf("%.2lf %.2lf, %.2lf %.2lf, %.2lf %.2lf\n", lx, 0.0, rx, 0.0, X, H);
    	}
    	printf("Case %d: %d\n", ++cases, (int) round(computeSkyline(segs, M)));
    	puts("");
    }
    return 0;
}
