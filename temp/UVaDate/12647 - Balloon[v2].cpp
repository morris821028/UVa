...
int touch[MAXN];
void preprocess(vector<Seg> segs) {
	int N = segs.size();
	for (int i = 0; i < N; i++) {
		toward[i] = stopX[i] = -1;
	...
	for (int i = 0; i < e.size(); i++) {
		...
		if (e[i].qkind) {
			...
			if (it != S.end() && it->id != e[i].id)
				toward[e[i].id] = it->id/*, printf("%d -> %d A\n", e[i].id, it->id)*/;
			else if (kt != S.end() && kt->id != e[i].id)
				toward[e[i].id] = kt->id/*, printf("%d -> %d B\n", e[i].id, kt->id)*/;
		}
		// 
		if (e[i].s == 1) {
	...
	for (int i = 0; i < N; i++) {
		Q.push_back(make_pair(stopY[i], i));
	} 
	sort(Q.begin(), Q.end());
	for (int j = N-1; j >= 0; j--) {
		int i = Q[j].second;
...
void query(vector<Seg> segs, int Qx[], int Q) { 
	int N = segs.size();
	for (int i = 0; i < N; i++) {
		if(segs[i].e < segs[i].s)
			swap(segs[i].s, segs[i].e);
	...
	set<Seg, CMP> S;	
	for (int i = 0; i < Q; i++) {
		e.push_back(Endpoint(Qx[i], 1, i, 1));
		touch[i] = -1;
	}
	for (int i = 0; i < N; i++) {
		e.push_back(Endpoint(segs[i].s.x, 1, i, 0));
		e.push_back(Endpoint(segs[i].e.x, -1, i, 0));
	}
	sort(e.begin(), e.end());
